...
.setThumbnail()
.addField(`HUNTER | Teşekkürler`, `**Selamlar, Ben HUNTER KİNG (HUNTER'in Geliştiricisi) Öncelikle Botumuzu Eklediğiniz ve Bize Destek Olduğunuz İçin Sizlere Teşekkürlerimi Sunarım**`)
.addField(`HUNTER | Prefix`, `**HUNTER Botun Prefixi(ön eki) = \`${fynx.prefix}\`\n\n Değiştirebilmek için \`${fynx.prefix}prefix\` Yazabilirsiniz.**`)
.addField(`HUNTER | Nasıl Kullanılır?`, `**HUNTER botun tüm özelliklerinden yararlanabilmek için sadece \`${fynx.prefix}yardım\` yazmanız yeterlidir.**`)
.addField(`HUNTER | Linkler`, `**Sohbet Kanalına hk!davet Yazmanız Yeterlidir**`)
.setFooter(`HUNTER  | Gelişmiş Türkçe Bot | 2020`)
.setTimestamp();
...
var oyun = [
"✨ Yardım almak için | hk!yardım",
  "hk!rapor | Hata bildir",
...
client.on('message', async msg => {
  ...
  if(msg.content == `<@!754040174236925952>`) return msg.channel.send(`> **HUNTER | Prefix**\n\n> <a:hypesquad:788769138486804551> **Sanırım beni etiketlediniz.**\n > <a:hypesquad:788769138486804551> **Buyurun prefix(ön ek)im** \`${prefix}\``);
});
...
.setDescription(`<a:tik:791965063606829066> <@!${message.author.id}> **adlı kullanıcı tarafından** <#${message.channel.id}> **kanalına gönderilen mesaj silindi!** \n\nSilinen Mesaj: **${message.content}**`)
  .setFooter("HUNTER Bot | Log Sistemi")
...
if (!modlogkanal) return;
  ...
  .setColor("RANDOM")
    ...
    .setDescription(`<a:banned:791966484745551872> **Üye Sunucudan Yasaklandı!** \n<@!${message.user.id}>, ${message.user.tag}`)
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                ...
                .setDescription(`<a:tik:791965063606829066> ${channel.name} **Adlı Metin Kanalı Oluşturuldu!**`)
                .setFooter(`HUNTER Bot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
...
.setTitle("SES KANALI OLUŞTURULDU")
                .setDescription(`<a:tik:791965063606829066> ${channel.name} **Adlı Ses Kanalı Oluşturuldu!**`)
                .setFooter(`HUNTER Bot | Log Sistemi Kanal ID: ${channel.id}`)
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                let embed = new Discord.MessageEmbed()
                    .setColor('RANDOM')
                .setDescription(`<a:tik:791965063606829066> ${channel.name} **Adlın Metin Kanalı  Silindi**`)
                .setFooter(`HUNTER Bot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
            if (channel.type === "voice") {
...
.setTitle("SES KANALI SİLİNDİ")
                .setDescription(`<a:tik:791965063606829066> ${channel.name} **Adlı Ses Kanalı Silindi**`)
            .setFooter(`HUNTER Bot | Log Sistemi  Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            }
    })
    ...
    if (küfür.some(word => message.content.toLowerCase().includes(word))) {
      try {
        if (!message.member.permissions.has('BAN_MEMBERS')) {
          ...
          return message.channel.send(`<a:nlem:788769138126487562> **Hey ${message.author} Dur! Bu Sunucuda Küfürü Engelliyorum!**`).then(message => message.delete(3000));
      ...
      try {
        if (!newMessage.member.permissions.has('BAN_MEMBERS')) {
         newMessage.delete();
          ...
          return newMessage.channel.send(`<a:nlem:788769138126487562> **Hey ${newMessage.author} Dur! Bu Sunucuda Küfürü Engelliyorum!**`).then(message => message.delete(3000));
    ...
    if (reklamengel.some(word => message.content.toLowerCase().includes(word))) {
      try {
        if (!message.member.permissions.has('BAN_MEMBERS')) {
          ...
          return message.channel.send(`<a:nlem:788769138126487562> **Hey ${message.author} Dur! Bu Sunucuda Reklamı Engelliyorum!**`).then(message => message.delete(3000));
      ...
      try {
        if (!newMessage.member.permissions.has('BAN_MEMBERS')) {
         newMessage.delete();
          ...
          return newMessage.channel.send(`<a:nlem:788769138126487562> **Hey ${newMessage.author} Dur! Bu Sunucuda Reklamı Engelliyorum!**`).then(message => message.delete(3000));
    ...
    if (i == 'acik') {
      if (msg.content.toLowerCase() == 'sa' || msg.content.toLowerCase() == 's.a' || msg.content.toLowerCase() == 'selamun aleyküm' || msg.content.toLowerCase() == 'sea' || msg.content.toLowerCase() == 's.a.' || msg.content.toLowerCase() == 'selam' || msg.content.toLowerCase() == 'slm') {
          try {
                  ...
                  return msg.reply('**<a:elsalla:791962297265291264> Aleyküm Selam, Hoşgeldin.** ')
          } catch(err) {
            console.log(err);
          }
      }
...
client.on("guildMemberAdd", async member => {
  ...
  ///....
  if (!mesaj) {
    return client.channels.cache.get(kanal).send("<a:giri:791967974365200415> `"+ member.user.username + "`**Adlı Kullanıcı Aramıza Katıldı!** `" + sayaç + "` **Kişi Olmamıza** `" + sonuç + "` **Kişi Kaldı.** `" + member.guild.memberCount + "` **Kişiyiz!**");
  }

  if (member.guild.memberCount == sayaç) {
    return client.channels.get(kanal).send(`<a:tik:791965063606829066> **Sayaç Sıfırlandı!** \`${member.guild.memberCount}\` **Kişiyiz!**`)
...
client.on("guildMemberRemove", async member => {
  ...
  if (!mesaj) {
    return client.channels.cache.get(kanal).send("<a:cikis:791968648377008158> `" + member.user.username + "` **Adlı Kullanıcı Aramızdan Ayrıldı.**`" + sayaç + "` **Kişi Olmamıza** `" + sonuç + "` **Kişi Kaldı.** `" + member.guild.memberCount + "` **Kişiyiz!**");
      }
...
client.on("guildMemberAdd", async member => {
  ...
  if (!mesaj) {
    client.channels.cache.get(kanal).send("<a:giri:791967974365200415> `" + member.user.username + "`** Hoş Geldin! Otomatik Rolün Verildi Seninle Beraber** `" + member.guild.memberCount + "` **Kişiyiz!**");
